...
class yolo_heatmap:
    ...
    def draw_detections(self, box, color, name, img):
        ...
        cv2.putText(img, str(name), (xmin, ymin - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.8, tuple(int(x) for x in color), 2, lineType=cv2.LINE_AA)  # Draw the category and confidence level
        return img
    ...
    def process(self, img_path, save_path):
        ...
        if self.show_result:
            cam_image = pred.plot(img=cam_image,
                                  conf=True, # Show confidence level
                                  font_size=None, # Font size, None is calculated based on the current image size
                                  line_width=None, # Line width, None is calculated based on the current image size
                                  labels=False, # Display Label
                                  )
        
        # Remove the padding boundary
...
def get_params():
    params = {
        'weight': 'yolo11n.pt', # Now only the weights need to be specified, and there is no need to specify the cfg
        'device': 'cuda:0',
        ...
        'ratio': 0.02, # 0.02-0.1
        'show_result': True, # There is no need to draw the result. Please set it to False
        'renormalize': False, # To limit the heat map within the box, please set it to True(valid only for detect,segment, and pose).
        'task':'detect', # task(detect,segment,pose,obb,classify)
